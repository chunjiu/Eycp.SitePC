...
define(function(require, exports, module) {
    ...
    //把日期转成月日
    String.prototype.DateFormatToMonth = function(formatDate){
        ...
        var setDate = o.M + '月' + o.d + '日';

        return setDate;
    }

    String.prototype.DateFormatToYear = function(formatDate){
     ...
     * @param {*} name 
     */
    String.prototype.getPara = function (name) {
        var _value = '';
        var _thisValue = this.toString();
        if(_thisValue.split('?') && _thisValue.split('?').length > 1) {
            var _this = _thisValue.split('?')[1].split('&');
            for(var i = 0; i < _this.length; i++) {
                if(_this[i]) {
                    var _tmp = _this[i].split('=');
                    if(_tmp[0] == name) {
                        _value = _tmp[1];
                        break;
                    }
